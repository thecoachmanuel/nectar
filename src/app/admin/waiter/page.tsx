...
export default function WaitersPage() {
  ...
  // Mock data
  const waiters = [
    { id: 1, name: "John Waiter", email: "johnw@example.com", phone: "+234 800 000 0000", status: "Active" },
    { id: 2, name: "Sarah Waiter", email: "sarahw@example.com", phone: "+234 811 111 1111", status: "Active" },
  ];
    ...
    <div className="pb-16">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#EFF0F6] mb-6">
        <div className="p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#EFF0F6]">
          <h3 className="font-semibold text-lg text-[#14142B]">Waiters</h3>
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left whitespace-nowrap">
            ...
            <tbody className="divide-y divide-[#EFF0F6]">
              {waiters.map((waiter) => (
                <tr key={waiter.id} className="hover:bg-[#FAFAFC] transition-colors">
                  <td className="px-6 py-4">
                    <span className="text-sm font-medium text-[#14142B]">{waiter.name}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm text-[#4E4B66]">{waiter.email}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm text-[#4E4B66]">{waiter.phone}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${waiter.status === 'Active' ? 'bg-[#E0FFED] text-[#1AB759]' : 'bg-[#FFEAEA] text-[#FB4E4E]'}`}>
                      {waiter.status}
                    </span>
                  </td>
