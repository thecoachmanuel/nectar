...
export default function DeliveryBoysPage() {
  ...
  // Mock data
  const deliveryBoys = [
    { id: 1, name: "David Express", email: "david@example.com", phone: "+234 822 222 2222", status: "Active" },
    { id: 2, name: "Sam Delivery", email: "sam@example.com", phone: "+234 833 333 3333", status: "Active" },
  ];
    ...
    <div className="pb-16">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#EFF0F6] mb-6">
        <div className="p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#EFF0F6]">
          <h3 className="font-semibold text-lg text-[#14142B]">Delivery Boys</h3>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <button className="h-10 px-4 rounded-xl bg-[#ff006b] text-white flex items-center gap-2 hover:bg-[#e60060] transition-colors shadow-md shadow-[#ff006b]/20">
              ...
              <span className="text-sm font-medium">Add Delivery Boy</span>
            </button>
          </div>
        </div>
          ...
          <table className="w-full text-left whitespace-nowrap">
            ...
            <tbody className="divide-y divide-[#EFF0F6]">
              {deliveryBoys.map((boy) => (
                <tr key={boy.id} className="hover:bg-[#FAFAFC] transition-colors">
                  <td className="px-6 py-4">
                    <span className="text-sm font-medium text-[#14142B]">{boy.name}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm text-[#4E4B66]">{boy.email}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm text-[#4E4B66]">{boy.phone}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${boy.status === 'Active' ? 'bg-[#E0FFED] text-[#1AB759]' : 'bg-[#FFEAEA] text-[#FB4E4E]'}`}>
                      {boy.status}
                    </span>
                  </td>
