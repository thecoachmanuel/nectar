...
export default function DeliveryBoysPage() {
  ...
  const fetchUsers = () => {
    execute("/api/admin/users?role=delivery_boy");
  };
  ...
  const confirmDelete = async () => {
    if (selectedUser) {
      await deleteUser(`/api/admin/users/${selectedUser._id}`, {
        method: "DELETE",
        successMessage: "Delivery boy deleted",
      });
      fetchUsers();
    }
  };
          ...
          <table className="w-full text-left whitespace-nowrap">
            ...
            <tbody className="divide-y divide-[#EFF0F6]">
              {loading && !users ? (
                ...
                <tr><td colSpan={5} className="p-8 text-center text-[#6E7191]">No delivery boys found</td></tr>
              ) : (
                users?.map((user: any) => (
