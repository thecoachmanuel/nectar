            ...
            {items.map((item) => {
              ...
              return (
                <div key={item._id} onClick={() => { setSelectedItem(item); setModalOpen(true); }}
                  className={`product-card-grid cursor-pointer overflow-hidden w-full min-w-0 ${isOutOfStock ? "opacity-80" : ""}`}>
                  <div className="relative pt-[75%] bg-[#f7f7fc] rounded-t-2xl overflow-hidden">
                    <img src={item.image || "/images/item/thumb.png"} alt={item.name}
                      className={`absolute inset-0 w-full h-full object-cover ${isOutOfStock ? "grayscale-[40%]" : ""}`}
